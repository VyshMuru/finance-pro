...
router.get('/', (req, res) => {
  ...
  const result = accounts.map(acc => {
    ...
    const currentBalance = isLiability
      ? acc.starting_balance - acc.total_inflows + acc.total_outflows
      : acc.starting_balance + acc.total_inflows - acc.total_outflows;
...
router.post('/', (req, res) => {
  ...
  const result = db.prepare('INSERT INTO accounts (name, type, starting_balance) VALUES (?, ?, ?)').run(name, type, starting_balance);
...
router.put('/:id', (req, res) => {
  ...
  db.prepare('UPDATE accounts SET name=?, type=?, starting_balance=? WHERE id=?').run(name, type, starting_balance, req.params.id);
