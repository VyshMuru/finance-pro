...
function AccountRow({ account, onUpdate, onDelete }) {
  ...
  const [form, setForm] = useState({ name: account.name, type: account.type, starting_balance: account.starting_balance });
  ...
  return (
    <tr className="table-row">
      ...
      <td className="px-4 py-3">
        <span className={clsx('badge', TYPE_BADGES[account.type] || TYPE_BADGES.other)}>{account.type}</span>
      </td>
      <td className="px-4 py-3 font-mono text-sm">{fmt.currency(account.starting_balance)}</td>
      <td className="px-4 py-3 font-mono text-sm text-green-400">{fmt.currency(account.total_inflows)}</td>
      <td className={clsx('px-4 py-3 font-mono text-sm font-semibold', isLiability ? 'text-red-400' : 'text-blue-400')}>
        {fmt.currency(account.current_balance)}
      </td>
      <td className="px-4 py-3">
        <div className="flex gap-1">
          <button onClick={() => setEditing(true)} className="p-1.5 rounded text-[var(--text-muted)] hover:text-blue-400 hover:bg-blue-500/10 transition-colors">
            <Pencil size={14}/>
          </button>
          <button onClick={() => onDelete(account.id)} className="p-1.5 rounded text-[var(--text-muted)] hover:text-red-400 hover:bg-red-500/10 transition-colors">
            <Trash2 size={14}/>
          </button>
        </div>
      </td>
...
export default function Accounts() {
  ...
  const [addForm, setAddForm] = useState({ name: '', type: 'checking', starting_balance: 0 });

  const load = async () => {
    setLoading(true);
    const [accs, sum] = await Promise.all([api.accounts.list(), api.accounts.summary()]);
    setAccounts(accs);
    setSummary(sum);
    setLoading(false);
  };
    ...
    <div className="flex flex-col gap-5 h-full overflow-y-auto pr-1">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <KPICard label="Total Accounts" value={summary?.totalAccounts || 0} color="text-cyan-400"
          ...
          <form onSubmit={handleAdd} className="flex gap-3 items-end p-3 bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)]">
            <div className="flex-1">
              ...
              <input value={addForm.name} onChange={e => setAddForm(f => ({ ...f, name: e.target.value }))}
                placeholder="e.g. TD Chequing" className="input text-sm" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" step="0.01" value={addForm.starting_balance}
        ...
        <div className="overflow-auto rounded-xl border border-[var(--border)]">
          <table className="w-full text-sm">
            <thead className="bg-[var(--bg-secondary)]">
              <tr>
                {['Account Name', 'Type', 'Starting Balance', 'Total Inflows', 'Current Balance', 'Actions'].map(h => (
                  <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] border-b border-[var(--border)] whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {accounts.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-8 text-[var(--text-muted)]">No accounts yet</td></tr>
              ) : accounts.map(acc => (
